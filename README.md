...
Qoder auto-loads `AGENTS.md` from the repo root as always-on context, so running ponytail from a checkout works with zero setup. For per-project rules, copy [`.qoder/rules/ponytail.md`](.qoder/rules/ponytail.md) into your project's `.qoder/rules/`. The bundled skills, including `ponytail-math`, are available via Qoder's Skill system; the plugin manifest at [`.qoder-plugin/plugin.json`](.qoder-plugin/plugin.json) points at the `skills/` directory.
...
Installs ponytail as an OpenClaw skill from ClawHub; the math, review, audit, debt, gain, and help skills install the same way (`clawhub install ponytail-math`, and so on). OpenClaw applies the base skill on coding tasks and also exposes it as a `/ponytail` command. Without ClawHub, copy the desired directory from [`.openclaw/skills/`](.openclaw/skills/) into `~/.openclaw/skills/`.
